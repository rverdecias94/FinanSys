...
### 4.2 INVARIANTE — Borrado de miembro de equipo (NO romper)
- **`removeMember` → RPC `owner_delete_team_member(p_member_id)`** (`SECURITY DEFINER`, gate `owner_id = auth.uid()`). Decisión de Roberto: a un miembro **"puro"** se le **elimina la cuenta `auth.users` por completo** (hard-delete) para que, al reintentar entrar, reciba "credenciales inválidas". Los **`audit_logs` NUNCA se borran** (se conserva `user_email`/`user_id`), así la auditoría del negocio sobrevive.
- **Clasificación "cuenta independiente" (NO destruir su auth):** se basa en **CONTENIDO REAL** — `EXISTS` en `transactions`/`products`/`movements` con `user_id = member`, **o** posee su propio equipo (`team_members.owner_id = member`), **o** es miembro activo de OTRO negocio. **NUNCA** contar una fila de `subscriptions` (una suscripción free residual NO implica independencia). Alinea con `check_email_availability`. ⚠️ Regresión histórica: incluir `subscriptions` en el guard hacía que miembros con suscripción free residual (creada por el bug de identidad) no se eliminaran y volvieran a entrar como titular free (`supabase/migrations/20260623_fix_owner_delete_member_independence.sql`).
- En el hard-delete se limpia el "cascarón" del miembro (incluida `subscriptions`) y se borra `auth.users`; las cuentas **independientes** solo se desligan (se conserva su auth).
