...
| P0.7 | ⬜ | **NUEVO (advisor)**: `auth_leaked_password_protection` desactivado (no comprueba contraseñas filtradas vía HaveIBeenPwned). **Activar en el dashboard** (Authentication → Providers → Password → "Leaked password protection"). Único toggle que no es SQL; lo hace Roberto. | `get_advisors(security)` WARN (sigue en 1). | xs |
...
| P1.2 | 🟠 **Modo solo-lectura no implementado**: un rol Consultor (sin `finanzas.edit`) puede editar y guardar desde el botón "Ver". `FinanzasMejorado.jsx:478` SÍ pasa `readonly`, pero `TransactionModal.jsx:75` no lo recibe ni lo usa (confirmado Sesión 3). | `TransactionModal.jsx` (no usa `readonly`); `FinanzasMejorado.jsx:142,478` | s |
...
| P1.5 | 🟠 **Admin delegado no puede crear roles**: `RoleModal` manda `owner_id = session.user.id` (miembro) y la RLS exige el id del dueño. | `RoleModal.jsx:88` vs `rbac_hardening_v2.sql:285-296` | s |
